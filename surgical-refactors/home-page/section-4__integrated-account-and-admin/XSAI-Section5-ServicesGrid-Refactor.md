# XSAI — SECTION 5: INTEGRATED ACCOUNTING & ADMIN — REFACTOR PROMPT
### Replace ServicesGrid.tsx entirely with the code below.

---

## THE PROBLEMS WITH THE CURRENT SECTION

1. **Two separate white cards with dot bullet lists** — visually communicates fragmentation. The entire message is "these are integrated" but two equal-weight separate boxes say the opposite.
2. **No section label, no eyebrow** — inconsistent with every other refactored section.
3. **No accent word on the headline** — all one colour, breaks the typographic pattern.
4. **Background is `#f9fafb`** — should be `#eff6ff` (blue-light) per the rotation. Blue differentiates this from the green sections and adds depth.
5. **Closing quote in italic quotation marks** — brand statement treated as a testimonial again.
6. **Green dot bullets give zero visual weight** to services like "Corporation tax" or "Companies House filing" — these represent real compliance value.
7. **No "one team" bridge** — the section makes no argument for why integration matters in practice. It just lists things.
8. **Both card icons are green** — Admin Support should use blue accent (`#3b82f6`) to create visual distinction between the two disciplines while keeping them unified.

---

## THE NEW DESIGN DIRECTION

**The key insight: Accounting and Admin aren't two products. They're two disciplines inside one infrastructure.** The layout must communicate unity, not separation.

**Structure:**
1. Section label + headline + sub-line
2. ONE unified panel card — Accounting left column, Admin right column, separated by a centre divider line, sharing one card background. Header shows they're under the same roof.
3. Service items become small icon + label tiles — no dot bullets
4. Below the unified card: a 3-column "one team" value strip — three short statements about what unified management means in practice
5. Closing brand statement as a proper dark typographic moment — not a floating quote

---

## FULL COMPONENT — REPLACE ServicesGrid.tsx
...
| Before | After |
|---|---|
| Two separate white cards — communicates fragmentation | One unified panel with internal column divider — communicates integration |
| All-green icons on both cards | Accounting uses blue (`#3b82f6`), Admin uses green — distinct but unified |
| Dot bullet lists | Icon + label service tiles with hover states |
| Background `#f9fafb` flat | `#eff6ff` blue-light with dual radial glows — blue top-left, green bottom-right |
| No section label | "Full Service" eyebrow in blue — matches the blue accent of this section |
| "Accounting" = no accent | Headline: "Integrated Accounting **& Admin**" — & Admin in green |
| Floating italic closing quote | Proper brand statement: large, bold, green accent, dot-rule above, supporting sub-line |
| No "why integration matters" | 3-column value strip: One point of contact / Aligned reporting / Consistent compliance |
| Admin column same height as Accounting but fewer items | Integration note fills the gap in the Admin column — adds copy that earns its space |

---

## MOBILE BEHAVIOUR

- Unified panel: grid collapses to single column on mobile — Accounting first, Admin below, separated by a top border (not side border)
- Service tiles inside each column: `grid-cols-1` on mobile for Accounting, single column for Admin — both stack cleanly
- Value strip: single column on mobile (`grid-cols-1`), 3-col on `md:`
- Section label in blue (`#3b82f6`) — clear blue-section signal even on small screens
- Closing statement: centred, full width, readable at mobile sizes
- Background glows: both visible on mobile but constrained by `overflow-hidden` on section
...
- `animate-pulse` on the panel header status dot — already in global CSS from hero
- Blue accent (`#3b82f6` / `#bfdbfe` / `#e0eeff` / `#eff6ff`) used consistently in this section only — maintains colour variety across the page
